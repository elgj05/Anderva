...
// ** Store & Actions
import { getData, deleteBusiness, getBusiness } from "@store/actions/businesses"
      ...
      cell: (row) => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            ...
            className="w-100"
            // onClick={() => store.dispatch(getBusiness(row.id))}
          >
...
const DataTableServerSide = () => {
  ...
  const onDelete = (row) => {
    dispatch(deleteBusiness(row.id, refresh))
  }

  const onEdit = (row) => {
    dispatch(getBusiness(row.id, toggleEditSidebar))
  }
    ...
    <Fragment>
      <Card>
        <CardHeader className="border-bottom">
          <CardTitle tag="h4">Business Data</CardTitle>
